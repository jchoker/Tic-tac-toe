...
namespace Choker.Challenges.Array
{
    partial class Array
    {
        ...
        public static string Tictactoe(int[][] moves)
        {
            ...
            {
                board[move[0], move[1]] = firstPlayer ? 'X' : 'O';
            ...
            {
                var ch = board[i, 0];
                if (ch != 0 && ch == board[i, 1] && ch == board[i, 2])
                    return PlayerName(ch);
            }
            for (int i = 0; i < size; i++) // check cols
            {
                var ch = board[0, i];
                if (ch != 0 && ch == board[1, i] && ch == board[2, i])
                    return PlayerName(ch);
            }
            ...
            {
                var ch = board[0, 0];
                if (ch != 0 && ch == board[1, 1] && ch == board[2, 2])
                    return PlayerName(ch);
            }
            {
                var ch = board[0, 2];
                if (ch != 0 && ch == board[1, 1] && ch == board[2, 0])
                    return PlayerName(ch);
            }

            return (moves.Length == size * size) ? "Draw" : "Pending";

            string PlayerName(char ch) => ch == 'X' ? "A" : "B";
        }
    }
}